...
func newSMTPClient(conn net.Conn, config *model.Config) (*smtp.Client, *model.AppError) {
	...
	} else if config.EmailSettings.ConnectionSecurity == model.CONN_SECURITY_NOTLS && config.EmailSettings.SMTPUsername != "" { // note: go library only supports PLAIN auth over non-tls connections
		if err = c.Auth(auth); err != nil {
			return nil, model.NewLocAppError("SendMail", "utils.mail.new_client.auth.app_error", nil, err.Error())
		}
    }
